...
if($_SERVER["REQUEST_METHOD"] == "POST"){           //確保請求是POST，處理表單提交的數據
    ...
    if($conn->connect_error){
        dir("連接資料庫失敗: " . $conn->connect_error);                    //如果連線失敗，終止程式並輸出錯誤訊息
    }
    ...
    if(empty($error_message)){
        $salt = random_bytes(16);                                       //生成16位元的salt，用來提高密碼安全性
        ...
        if($conn->query($sql) === TRUE){
            ...
            echo '<button onclick = "window.location.href = \'loginWeb.php\'">錢網登入頁面</button>';       //註冊成功就顯示按鈕讓用戶前往登入頁面
        }
        else{
